...
Gatt.prototype.onAclStreamData = function(cid, data) {
  ...
  if (this._currentCommand && data.toString('hex') === this._currentCommand.buffer.toString('hex')) {
    ...
    var attEcode = null
    if (data[0] === ATT_OP_ERROR) { attEcode = data[4] }
...
Gatt.prototype.read = function(serviceUuid, characteristicUuid) {
  ...
  var callback = function(data, securityError) {
    var opcode = data[0];
    if (securityError) {
      this.emit('read', this._address, serviceUuid, characteristicUuid, data[4], true);
    } else if (opcode === ATT_OP_READ_RESP || opcode === ATT_OP_READ_BLOB_RESP) {
      ...
      if (data.length === this._mtu) {
        ...
        this.emit('read', this._address, serviceUuid, characteristicUuid, readData, false);
      }
    } else {
      this.emit('read', this._address, serviceUuid, characteristicUuid, readData, false);
    }
...
Gatt.prototype.write = function(serviceUuid, characteristicUuid, data, withoutResponse) {
  ...
  if (withoutResponse) {
    ...
    this._queueCommand(this.writeRequest(characteristic.valueHandle, data, false), function(data, securityError) {
      var opcode = data[0];
      if (securityError) {
        this.emit('write', this._address, serviceUuid, characteristicUuid, data[4], true);
      } else if (opcode === ATT_OP_WRITE_RESP) {
        this.emit('write', this._address, serviceUuid, characteristicUuid);
      }
    }.bind(this));
  }
...
Gatt.prototype.notify = function(serviceUuid, characteristicUuid, notify) {
  ...
  this._queueCommand(this.readByTypeRequest(characteristic.startHandle, characteristic.endHandle, GATT_CLIENT_CHARAC_CFG_UUID), function(data) {
    ...
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      ...
      this._queueCommand(this.writeRequest(handle, valueBuffer, false), function(data, securityError) {
        var opcode = data[0];
        if (securityError) {
          this.emit('notify', this._address, serviceUuid, characteristicUuid, data[4], true);
        } else if (opcode === ATT_OP_WRITE_RESP) {
