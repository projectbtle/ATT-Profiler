...
NobleBindings.prototype.onRead = function(address, serviceUuid, characteristicUuid, data, isSecurityError=false) {
  var uuid = address.split(':').join('').toLowerCase();
  /* Profiler Code CHANGE: isSecurityError is specific to Profiler */
  this.emit('read', uuid, serviceUuid, characteristicUuid, data, false, isSecurityError);
};
...
NobleBindings.prototype.onWrite = function(address, serviceUuid, characteristicUuid, error=null, isSecurityError=false) {
  var uuid = address.split(':').join('').toLowerCase();

  this.emit('write', uuid, serviceUuid, characteristicUuid, error, isSecurityError);
};
...
NobleBindings.prototype.onNotify = function(address, serviceUuid, characteristicUuid, state, isSecurityError=false) {
  var uuid = address.split(':').join('').toLowerCase();

  this.emit('notify', uuid, serviceUuid, characteristicUuid, state, isSecurityError);
};
